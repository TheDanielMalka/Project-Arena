...
class TestMatchGating:
    ...
    def test_join_cs2_match_with_steam_returns_200(self):
        ...
        ctx, session = _make_session_mock()
        session.execute.return_value.fetchone.side_effect = [
            ("CS2", "waiting", None, "CRYPTO", None, 2, 1, "custom", None),  # match lookup
    ...
    def test_join_valorant_match_without_riot_returns_403(self):
        ...
        ctx, session = _make_session_mock()
        session.execute.return_value.fetchone.side_effect = [
            ("Valorant", "waiting", None, "CRYPTO", None, 2, 1, "custom", None),  # match lookup
    ...
    def test_join_match_not_waiting_returns_409(self):
        ...
        ctx, session = _make_session_mock()
        session.execute.return_value.fetchone.side_effect = [
            ("CS2", "in_progress", None, "CRYPTO", None, 2, 1, "custom", None),  # match already started → 409
        ]
        with patch("main.SessionLocal", return_value=ctx):
            resp = client.post(
                f"/matches/{match_id}/join",
...
class TestWithdrawArenaTokens:
    ...
    def test_withdraw_standard_rate_returns_200(self):
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["at_burned"] == 1050
        assert data["usdt_value"] == 10.0
        assert "wallet_address" in data
        assert data["at_balance"] == 3950

    def test_withdraw_discount_rate_returns_200(self):
        """2100 AT → $20 USDT (two chunks of 1050)."""
        ctx, session = _make_session_mock()
        session.execute.return_value.fetchone.side_effect = [
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["at_burned"] == 2100
        assert data["usdt_value"] == 20.0
    ...
    def test_withdraw_invalid_multiple_returns_400(self):
        """Amount not a multiple of 1050 → 400."""
        ctx, session = _make_session_mock()
        with patch("main.SessionLocal", return_value=ctx):
            resp = client.post(
                "/wallet/withdraw-at",
                json={"at_amount": 500},  # 500 not divisible by 1050
                headers=self._auth_header(),
            )
        assert resp.status_code == 400
    ...
    def test_withdraw_daily_limit_exceeded_returns_429(self):
        ...
        session.execute.return_value.fetchone.return_value = self._user_row(
            balance=20000, daily=9450  # 9450 used today, 1050 more = 10500 > 10000
        )
        with patch("main.SessionLocal", return_value=ctx):
            resp = client.post(
                "/wallet/withdraw-at",
