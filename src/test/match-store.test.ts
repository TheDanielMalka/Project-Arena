...
describe("matchStore", () => {
  ...
  it("2v2 match activates automatically when 4th player deposits", () => {
    // c4: 2v2, teamA full (2), teamB empty, depositsReceived=2 — needs 2 more
    useMatchStore.getState().joinMatch("c4", "Player3", "B");
    expect(useMatchStore.getState().matches.find((m) => m.id === "c4")!.status).toBe("waiting");

    useMatchStore.getState().joinMatch("c4", "Player4", "B");
    expect(useMatchStore.getState().matches.find((m) => m.id === "c4")!.status).toBe("in_progress");
  });

  it("4v4 match stays waiting until all 8 players deposit", () => {
    ...
    expect(useMatchStore.getState().matches.find((m) => m.id === "c5")!.status).toBe("waiting");

    store.joinMatch("c5", "B4", "B");
    expect(useMatchStore.getState().matches.find((m) => m.id === "c5")!.status).toBe("in_progress");
  });
